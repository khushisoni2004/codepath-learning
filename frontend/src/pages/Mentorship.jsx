...
export default function Mentorship() {
  ...
  async function submitBooking(event) {
    ...
    const payerUpiId = form.payerUpiId.trim().toLowerCase().replace(/\s+/g, "");
    const transactionId = form.transactionId.trim().toUpperCase().replace(/\s+/g, "");
    if (!payerUpiId && !transactionId) return setMessage("Enter either a valid UPI ID or transaction ID/UTR.");
    if (payerUpiId && !/^[a-z0-9._-]{2,100}@[a-z0-9.-]{2,64}$/.test(payerUpiId)) return setMessage("Enter a valid UPI ID, for example name@bank.");
    if (transactionId && !/^(?=.*\d)[A-Z0-9-]{9,40}$/.test(transactionId)) return setMessage("Enter a valid transaction ID/UTR.");
    setMessage("");
    try {
      const token = localStorage.getItem("codepathAuthToken");
      const response = await apiFetch(`${API_URL}/mentorship/bookings`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ topic: form.topic, payerUpiId, transactionId }) });
      const data = await response.json().catch(() => ({}));
  ...
  return (
    <main className="mentorship-page">
      ...
      {status && status.status !== "PAID" ? <section className="mentorship-status-section"><div className="container mentorship-status-card"><div><span className="mentorship-page-label">REQUEST STATUS</span><h2>{status.status === "REJECTED" ? "Payment requires review again" : "Booking request submitted"}</h2><p>{status.status === "REJECTED" ? "Your request was not approved. Please submit a valid UPI ID or transaction ID." : "Our team will verify your manual payment. WhatsApp access will unlock only after admin approval."}</p></div><button type="button" onClick={refreshStatus} className="mentorship-outline-button">Check Verification Status</button></div></section> : null}
