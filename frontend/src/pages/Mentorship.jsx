...
export default function Mentorship() {
  ...
  async function submitBooking(event) {
    ...
    const transactionId = form.transactionId.replace(/\D/g, "");
    if (!/^\d{12}$/.test(transactionId)) {
      setMessage("Enter the valid 12-digit bank UTR/RRN shown after completing the QR payment.");
      return;
    }
    setMessage("");
    try {
      const token = localStorage.getItem("codepathAuthToken");
      const response = await apiFetch(`${API_URL}/mentorship/bookings`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ topic: form.topic, transactionId }) });
      const data = await response.json().catch(() => ({}));
